...
exports.getAllBooksByAuthor = async (req, res) => {
  try {
    ...
    if (bookSnapshots.length === 0) {
      return res
        .status(200)
        .json({ message: "No books found for this author", books: [] });
    }

    const booksPromises = bookSnapshots.map((book) =>
      db.collection("books").doc(book.id).get()
    );
    const bookDocs = await Promise.all(booksPromises);
    const books = bookDocs.map((bookSnap) => ({
      id: bookSnap.id,
      ...bookSnap.data(),
    }));
  ...
  try {
    ...
    if (!authorId || !title || !genre || !year || !description || !pages) {
      return res.status(400).json({
        error: "All fields are required, including authorId",
    ...
    for (const [key, result] of Object.entries(validators)) {
      if (result !== true) {
        return res
          .status(400)
          .json({ message: `${key} is invalid: ${result}` });
      }
    }
    ...
    await db.collection("books").doc(bookData.id).set(bookData);
    await authorRef.update({
      books: admin.firestore.FieldValue.arrayUnion({
        id: bookData.id,
...
exports.updateBook = async (req, res) => {
  try {
    ...
    if (!authorId || !bookId) {
      return res
        .status(400)
        .json({ error: "Both authorId and bookId are required" });
    }
    ...
    //modificam si in lista autorului 
    ...
    res.status(200).json({
      ...
      updatedBook: { id: bookId, ...updateData },
    });
  } catch (error) {
    res
      .status(500)
